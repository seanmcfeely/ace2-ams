...
def test_read_all(db):
    ...
    result = crud.submission_tool.read_all(db=db)
    assert len(result) == 2
...
def test_read_by_uuid(db):
    ...
    result = crud.submission_tool.read_by_uuid(uuid=obj.uuid, db=db)
...
def test_read_by_value(db):
    ...
    result = crud.submission_tool.read_by_value(value=obj.value, db=db)
    assert result.uuid == obj.uuid
