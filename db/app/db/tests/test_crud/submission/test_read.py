...
def test_read_observables(db):
    ...
    # Ensure the exception is raised if you give a nonexistent UUID
    with pytest.raises(UuidNotFoundInDatabase):
        crud.submission.read_observables(uuids=[uuid4()], db=db)


def test_read_submission_tree(db):
    submission = factory.submission.create_from_json_file(
        db=db, json_path="/app/db/tests/alerts/small.json", submission_name="Test Alert"
    )
...
def test_circular_tree(db):
    ...
    submission = factory.submission.create_from_json_file(
        db=db, json_path="/app/db/tests/alerts/circular.json", submission_name="Circular Alert"
    )
...
def test_critical_path(db):
    ...
    * - critical path
    ...
    submission = factory.submission.create_from_json_file(
        db=db, json_path="/app/db/tests/alerts/critical_path.json", submission_name="Critical Path Alert"
    )
