...
def test_create(db):
    obj = crud.metadata_display_value.create_or_read(model=MetadataDisplayValueCreate(value="test value"), db=db)
...
def test_create_duplicate_value(db):
    obj1 = crud.metadata_display_value.create_or_read(model=MetadataDisplayValueCreate(value="test"), db=db)
    assert obj1

    # Ensure that you get the same object back if you try to create a duplicate value
    obj2 = crud.metadata_display_value.create_or_read(model=MetadataDisplayValueCreate(value=obj1.value), db=db)
