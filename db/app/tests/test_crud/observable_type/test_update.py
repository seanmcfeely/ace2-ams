...
def test_update(db):
    ...
    crud.observable_type.update(
        uuid=obj.uuid,
        model=ObservableTypeUpdate(description="test description", value="new value"),
        db=db,
    )
...
def test_update_duplicate_value(db):
    ...
    result = crud.observable_type.update(uuid=obj2.uuid, model=ObservableTypeUpdate(value=obj1.value), db=db)
