...
def test_update_invalid_fields(client_valid_access_token, key, value):
    update = client_valid_access_token.patch(f"/api/event/type/{uuid.uuid4()}", json={key: value})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_update_invalid_uuid(client_valid_access_token):
    update = client_valid_access_token.patch("/api/event/type/1", json={"value": "test"})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_update_nonexistent_uuid(client_valid_access_token):
    update = client_valid_access_token.patch(f"/api/event/type/{uuid.uuid4()}", json={"value": "test"})
...
def test_update_queue(client_valid_access_token, db):
    ...
    # Create the object
    obj = helpers.create_event_type(value="test", queues=["default"], db=db)

    # Update it
    update = client_valid_access_token.patch(f"/api/event/type/{obj.uuid}", json={"queues": ["updated"]})
    assert update.status_code == status.HTTP_204_NO_CONTENT
