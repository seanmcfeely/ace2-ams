...
def test_get_all(client_valid_access_token, db):
    # Create some objects
    helpers.create_event_source(value="test", queues=["internal"], db=db)
    helpers.create_event_source(value="test2", queues=["external"], db=db)
    helpers.create_event_source(value="test3", queues=["internal", "external"], db=db)
