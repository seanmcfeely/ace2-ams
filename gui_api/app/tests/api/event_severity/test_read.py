...
def test_get_all_event_severities(client_valid_access_token, requests_mock):
    ...
    requests_mock.get(
        f"http://db-api/api/event/severity/?{params}", json={"items": [], "total": 0, "limit": 50, "offset": 0}
    )

    client_valid_access_token.get(f"/api/event/severity/?{params}")
    ...
    assert unquote_plus(requests_mock.request_history[1].url) == unquote_plus(
        f"http://db-api/api/event/severity/?{params}"
    )
