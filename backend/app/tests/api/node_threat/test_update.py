...
def test_update_invalid_fields(client_valid_token, key, value):
    update = client_valid_token.patch(f"/api/node/threat/{uuid.uuid4()}", json={key: value})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_update_invalid_uuid(client_valid_token):
    update = client_valid_token.patch("/api/node/threat/1", json={"types": ["test_type"], "value": "test"})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_update_duplicate_unique_fields(client_valid_token, key):
    ...
    client_valid_token.post("/api/node/threat/", json=create1_json)

    create2_json = {"types": ["test_type"], "value": "test2"}
    create2 = client_valid_token.post("/api/node/threat/", json=create2_json)
...
def test_update_nonexistent_uuid(client_valid_token):
    update = client_valid_token.patch(f"/api/node/threat/{uuid.uuid4()}", json={"types": ["test_type"], "value": "test"})
...
def test_update_valid_types(client_valid_token, value):
    # Create some node threat types
    initial_types = ["test_type1", "test_type2", "test_type3"]
    for threat_type in initial_types:
        client_valid_token.post("/api/node/threat/type/", json={"value": threat_type})

    # Create the object
    create = client_valid_token.post("/api/node/threat/", json={"types": initial_types, "value": "test"})
    ...
    # Update it
    update = client_valid_token.patch(create.headers["Content-Location"], json={"types": value})
...
def test_update(client_valid_token, key, initial_value, updated_value):
    ...
    create = client_valid_token.post("/api/node/threat/", json=create_json)
    assert create.status_code == status.HTTP_201_CREATED
