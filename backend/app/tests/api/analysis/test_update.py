...
def test_update_invalid_fields(client_valid_token, key, value):
    update = client_valid_token.patch(f"/api/analysis/{uuid.uuid4()}", json={key: value, "version": str(uuid.uuid4())})
...
def test_update_invalid_node_fields(client_valid_token, key, value):
    update = client_valid_token.patch(f"/api/analysis/{uuid.uuid4()}", json={"version": str(uuid.uuid4()), key: value})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
    assert key in update.text


def test_update_invalid_uuid(client_valid_token):
    update = client_valid_token.patch("/api/analysis/1", json={"version": str(uuid.uuid4())})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_update_nonexistent_analysis_module_type(client_valid_token):
    # Create an analysis
    version = str(uuid.uuid4())
    create = client_valid_token.post("/api/analysis/", json={"version": version})
...
def test_update_nonexistent_uuid(client_valid_token):
    update = client_valid_token.patch(f"/api/analysis/{uuid.uuid4()}", json={"version": str(uuid.uuid4())})
    assert update.status_code == status.HTTP_404_NOT_FOUND
...
def test_update_analysis_module_type(client_valid_token):
...
def test_update(client_valid_token, key, initial_value, updated_value):
    ...
    create = client_valid_token.post("/api/analysis/", json=create_json)
    assert create.status_code == status.HTTP_201_CREATED
