...
def test_get_invalid_uuid(client_valid_token):
    get = client_valid_token.get("/api/event/vector/1")
    assert get.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_get_nonexistent_uuid(client_valid_token):
    get = client_valid_token.get(f"/api/event/vector/{uuid.uuid4()}")
...
def test_get_all(client_valid_token):
    ...
    client_valid_token.post("/api/event/vector/", json={"value": "test2"})

    # Read them back
    get = client_valid_token.get("/api/event/vector/")
