...
def test_get_invalid_uuid(client_valid_token):
    get = client_valid_token.get("/api/event/type/1")
    assert get.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_get_nonexistent_uuid(client_valid_token):
    get = client_valid_token.get(f"/api/event/type/{uuid.uuid4()}")
...
def test_get_all(client_valid_token):
    ...
    client_valid_token.post("/api/event/type/", json={"value": "test2"})

    # Read them back
    get = client_valid_token.get("/api/event/type/")
