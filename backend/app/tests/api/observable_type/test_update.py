...
def test_update_invalid_fields(client_valid_token, key, value):
    update = client_valid_token.patch(f"/api/observable/type/{uuid.uuid4()}", json={key: value})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_update_invalid_uuid(client_valid_token):
    update = client_valid_token.patch("/api/observable/type/1", json={"value": "test"})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_update_duplicate_unique_fields(client_valid_token, key):
    ...
    client_valid_token.post("/api/observable/type/", json=create1_json)

    create2_json = {"value": "test2"}
    create2 = client_valid_token.post("/api/observable/type/", json=create2_json)
...
def test_update_nonexistent_uuid(client_valid_token):
    update = client_valid_token.patch(f"/api/observable/type/{uuid.uuid4()}", json={"value": "test"})
...
def test_update(client_valid_token, key, initial_value, updated_value):
    ...
    create = client_valid_token.post("/api/observable/type/", json=create_json)
