...
def test_delete_invalid_uuid(client_valid_token):
    delete = client_valid_token.delete("/api/user/1")
    assert delete.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_delete_nonexistent_uuid(client_valid_token):
    delete = client_valid_token.delete(f"/api/user/{uuid.uuid4()}")
...
def test_delete(client_valid_token):
    ...
    alert_queue_create = client_valid_token.post("/api/alert/queue/", json={"value": "test_queue"})

    # Create a user role
    user_role_create = client_valid_token.post("/api/user/role/", json={"value": "test_role"})
    ...
    get = client_valid_token.get(alert_queue_create.headers["Content-Location"])
    assert get.status_code == status.HTTP_200_OK
    assert get.json()["value"] == "test_queue"

    # Make sure the user role is still there
    get = client_valid_token.get(user_role_create.headers["Content-Location"])
    assert get.status_code == status.HTTP_200_OK
