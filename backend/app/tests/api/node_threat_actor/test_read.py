...
def test_get_invalid_uuid(client_valid_access_token):
    get = client_valid_access_token.get("/api/node/threat_actor/1")
    assert get.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_get_nonexistent_uuid(client_valid_access_token):
    get = client_valid_access_token.get(f"/api/node/threat_actor/{uuid.uuid4()}")
...
def test_get_all(client_valid_access_token, db):
    # Create some objects
    helpers.create_node_threat_actor(value="test", queues=["internal"], db=db)
    helpers.create_node_threat_actor(value="test2", queues=["external"], db=db)
    helpers.create_node_threat_actor(value="test3", queues=["internal", "external"], db=db)
