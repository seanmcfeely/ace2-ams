...
def test_delete_invalid_uuid(client_valid_token):
    delete = client_valid_token.delete("/api/event/status/1")
    assert delete.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_delete_nonexistent_uuid(client_valid_token):
    delete = client_valid_token.delete(f"/api/event/status/{uuid.uuid4()}")
...
def test_delete(client_valid_token):
    # Create the object
    create = client_valid_token.post("/api/event/status/", json={"value": "test"})
