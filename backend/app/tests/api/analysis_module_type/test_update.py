...
def test_update_invalid_fields(client_valid_token, key, value):
    update = client_valid_token.patch(f"/api/analysis/module_type/{uuid.uuid4()}", json={key: value})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_update_invalid_uuid(client_valid_token):
    update = client_valid_token.patch("/api/analysis/module_type/1", json={"value": "test_type"})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_update_duplicate_value_version(client_valid_token):
    # Create some objects
    client_valid_token.post("/api/analysis/module_type/", json={"value": "test", "version": "1.0.0"})
    create = client_valid_token.post("/api/analysis/module_type/", json={"value": "test", "version": "1.0.1"})

    # Ensure you cannot update an analysis module type to have a duplicate version+value combination
    update = client_valid_token.patch(create.headers["Content-Location"], json={"version": "1.0.0"})
    assert update.status_code == status.HTTP_409_CONFLICT


def test_update_nonexistent_uuid(client_valid_token):
    update = client_valid_token.patch(f"/api/analysis/module_type/{uuid.uuid4()}", json={"value": "test"})
...
def test_update_valid_list_fields(client_valid_token, api_endpoint, key, values):
    # Create the analysis module type
    create = client_valid_token.post("/api/analysis/module_type/", json={"value": "test", "version": "1.0.0"})
    ...
    for value in list(set(values)):
        client_valid_token.post(api_endpoint, json={"value": value})

    # Update it
    update = client_valid_token.patch(create.headers["Content-Location"], json={key: values})
...
def test_update(client_valid_token, key, initial_value, updated_value):
    ...
    create = client_valid_token.post("/api/analysis/module_type/", json=create_json)
    assert create.status_code == status.HTTP_201_CREATED
