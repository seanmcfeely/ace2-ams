...
<template>
  ...
  <div v-if="props.field === 'name'">
    <img v-if="alertIconPath" data-cy="alert-icon" src="@/assets/alertIcons/test.png" />
    ...
    <!-- Alert Tags -->
    <span data-cy="tags">
      <MetadataTag v-for="tag in getAllAlertTags(props.data)" :key="tag.uuid" :tag="tag"></MetadataTag>
    </span>
    <!-- Alert comments -->
  ...
  <div v-else-if="props.field === 'comments'">
    ...
    <Comment v-for="comment in props.data.comments" v-else :key="comment.uuid" :comment="comment"
      :include-time="true" />
  </div>

  <!-- Alert Disposition -->
  <div v-else-if="props.field === 'disposition'">
    <AlertDispositionTag :disposition="props.data[props.field]"></AlertDispositionTag>
  </div>
...
const props = defineProps({
  ...
  field: { type: String as PropType<Extract<keyof alertSummary, string>>, required: true },
});

const alertIconPath = computed(() => {
  if (config && config.alerts) {
    if (props.data.type in config.alerts.alertIconTypeMapping) {
      return `@/assets/alertIcons/${config.alerts.alertIconTypeMapping[props.data.type]
        }`;
    }
  }
  return undefined;
