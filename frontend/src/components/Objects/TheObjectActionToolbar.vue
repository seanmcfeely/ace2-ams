<!-- ObjectActionToolbar.vue -->
<!-- Toolbar containing object actions, such as Assign, Comment, Tag, etc. -->
...
<script setup lang="ts">
  ...
  const props = defineProps({
    reloadObject: {
      type: String as PropType<"object" | "table">,
      required: true,
    },
  ...
  const ownedByCurrentUser = computed(() => {
    if (props.reloadObject == "object" && objectStore.open) {
      if ("owner" in objectStore.open) {
        const authStore = useAuthStore();
        return objectStore.open.owner?.uuid == authStore.user.uuid;
      }
    }
    return false;
  });
  ...
  const requestReload = () => {
    if (props.reloadObject == "table") {
      ...
      objectStore.requestReload = true;
    }
  };
