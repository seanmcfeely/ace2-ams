  ...
  >
    ...
    <div class="field col-fixed" style="width: 150px">
      <MultiSelect
        ...
        append-to="self"
        :options="threatTypeStore.allItems"
        option-label="value"
...
<script setup lang="ts">
  ...
  const props = defineProps({
    modelValue: { type: Array as PropType<threatRead[]>, required: true },
  ...
  const newThreatTypes = ref<threatTypeRead[]>([]);
  const selectedThreats = ref<threatRead[]>(props.modelValue);
  const showEditThreat = ref(false);

  const formatThreatTypes = (threatTypes: threatTypeRead[]) => {
  ...
  const openEditThreatPanel = (threat: threatRead | null) => {
    ...
    if (threat) {
      ...
      // that also exist in the threatTypeStore
      newThreatTypes.value = threatTypeStore.allItems.filter(
        (threatTypeOption) =>
          threat.types.some(
  ...
  const submitNewThreat = async () => {
    try {
      if (editingExistingThreat.value && editingExistingThreatUuid.value) {
        await threatStore.update(editingExistingThreatUuid.value, {
          types: newThreatTypes.value.map((x) => x.value),
        });
      } else if (newThreatName.value) {
        await threatStore.create({
