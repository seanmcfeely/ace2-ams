...
def test_delete_invalid_uuid(client):
    delete = client.delete("/api/metadata/display_value/1")
    assert delete.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_delete_nonexistent_uuid(client):
    delete = client.delete(f"/api/metadata/display_value/{uuid.uuid4()}")
...
def test_delete_used(client, db):
    ...
    factory.observable.create_or_read(
        type="type", value="value", parent_analysis=submission.root_analysis, display_value="test", db=db
    )

    # Ensure you cannot delete it now that it is in use
    delete = client.delete(f"/api/metadata/display_value/{obj.uuid}")
