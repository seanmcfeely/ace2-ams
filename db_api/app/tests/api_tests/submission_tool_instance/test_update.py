...
def test_update_invalid_fields(client, key, value):
    update = client.patch(f"/api/submission/tool/instance/{uuid.uuid4()}", json={key: value})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_update_invalid_uuid(client):
    update = client.patch("/api/submission/tool/instance/1", json={"value": "test"})
    assert update.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_update_duplicate_unique_fields(client, db, key):
    ...
    update = client.patch(f"/api/submission/tool/instance/{obj2.uuid}", json={key: getattr(obj1, key)})
    assert update.status_code == status.HTTP_400_BAD_REQUEST


def test_update_nonexistent_uuid(client):
    update = client.patch(f"/api/submission/tool/instance/{uuid.uuid4()}", json={"value": "test"})
...
def test_update(client, db, key, initial_value, updated_value):
    ...
    # Update it
    update = client.patch(f"/api/submission/tool/instance/{obj.uuid}", json={key: updated_value})
