...
def test_create_invalid_fields(client, key, value):
    ...
    create = client.post("/api/metadata/tag/", json=create_json)
...
def test_create_valid_optional_fields(client, key, value):
    # Create the object
    create = client.post("/api/metadata/tag/", json={key: value, "value": "test"})
...
def test_create_valid_required_fields(client):
    # Create the object
    create = client.post("/api/metadata/tag/", json={"value": "test"})
    assert create.status_code == status.HTTP_201_CREATED
