...
def test_create(db):
    ...
    obj = crud.submission_comment.create_or_read(
        model=SubmissionCommentCreate(node_uuid=submission.uuid, username="analyst", value="test"), db=db
    )
...
def test_create_duplicate_value(db):
    ...
    obj2 = crud.submission_comment.create_or_read(
        model=SubmissionCommentCreate(node_uuid=submission.uuid, username="analyst", value=obj1.value), db=db
    )
