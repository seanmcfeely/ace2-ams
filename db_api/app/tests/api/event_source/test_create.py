...
def test_create_nonexistent_queue(client):
    create = client.post("/api/event/source/", json={"value": "test", "queues": ["nonexistent_queue"]})
...
def test_create_valid_optional_fields(client, db, key, value):
    ...
    create = client.post("/api/event/source/", json={key: value, "value": "test", "queues": ["test_queue"]})
...
def test_create_valid_required_fields(client, db):
    # Create the object
    factory.queue.create_or_read(value="test_queue", db=db)
    create = client.post("/api/event/source/", json={"value": "test", "queues": ["test_queue"]})
    assert create.status_code == status.HTTP_201_CREATED
