...
def test_update_duplicate_unique_fields(client, db, key):
    ...
    update = client.patch(f"/api/node/threat/type/{obj2.uuid}", json={key: getattr(obj1, key)})
...
def test_update(client, db, key, initial_value, updated_value):
    ...
    # Update it
    update = client.patch(f"/api/node/threat/type/{obj.uuid}", json={key: updated_value})
...
def test_update_queue(client, db):
    ...
    # Update it
    update = client.patch(f"/api/node/threat/type/{obj.uuid}", json={"queues": ["updated"]})
    assert update.status_code == status.HTTP_204_NO_CONTENT
