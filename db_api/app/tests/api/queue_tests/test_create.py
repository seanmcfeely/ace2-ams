...
def test_create_invalid_fields(client, key, value):
    ...
    create = client.post("/api/queue/", json=create_json)
...
def test_create_duplicate_unique_fields(client, key):
    ...
    client.post("/api/queue/", json=create1_json)

    # Ensure you cannot create another object with the same unique field value
    create2_json = {"value": "test2"}
    create2_json[key] = create1_json[key]
    create2 = client.post("/api/queue/", json=create2_json)
...
def test_create_valid_optional_fields(client, key, value):
    # Create the object
    create = client.post("/api/queue/", json={key: value, "value": "test"})
...
def test_create_valid_required_fields(client):
    # Create the object
    create = client.post("/api/queue/", json={"value": "test"})
    assert create.status_code == status.HTTP_201_CREATED
