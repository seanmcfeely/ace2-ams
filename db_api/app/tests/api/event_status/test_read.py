...
def test_get_all(client_valid_access_token, db):
    # Create some objects
    helpers.create_event_status(value="test", queues=["internal"], db=db)
    helpers.create_event_status(value="test2", queues=["external"], db=db)
    helpers.create_event_status(value="test3", queues=["internal", "external"], db=db)
