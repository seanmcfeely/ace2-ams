...
def test_get_invalid_uuid(client):
    get = client.get("/api/observable/relationship/type/1")
    assert get.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_get_nonexistent_uuid(client):
    get = client.get(f"/api/observable/relationship/type/{uuid.uuid4()}")
...
def test_get_all(client, db):
    ...
    # Read them back
    get = client.get("/api/observable/relationship/type/")
