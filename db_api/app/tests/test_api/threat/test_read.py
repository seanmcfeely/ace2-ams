...
def test_get_all(client, db):
    # Create some objects
    factory.threat.create_or_read(value="test", queues=["internal"], types=["test_type"], db=db)
    factory.threat.create_or_read(value="test2", queues=["external"], types=["test_type"], db=db)
