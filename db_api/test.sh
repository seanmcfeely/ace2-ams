...
docker rm -f ace2-db-api-test > /dev/null 2>&1
...
cd ..
docker build -t ace2-db-api-test -f db_api/Dockerfile .
docker run -d --net ace2-db-test-net --name ace2-db-api-test -e DATABASE_URL=$DATABASE_URL ace2-db-api-test > /dev/null

# Run the tests inside the Python container and capture its return code
echo "Running tests"
docker exec ace2-db-api-test pytest -p no:cacheprovider -vv ${NEW_PATH:-tests/}
RETURN_CODE=$?

# Cleanup
echo "Cleaning up"
docker rm -f ace2-db-api-test > /dev/null
